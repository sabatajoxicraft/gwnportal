...
/**
 * Resolve and parse coordinates from a Google Maps URL.
 *
 * Prefers actual place coordinates (!3d...!4d...) over viewport centre (@lat,lng)
 * so that map/street-view previews land on the property, not the viewport midpoint.
 *
 * @param  string      $map_url   Raw URL stored in the database (may be a shortened link).
 * @param  string      $map_host  Lower-cased host already extracted by the caller.
 * @return array{lat:string|null, lng:string|null, map_query:string, parse_url:string}
 */
function parseGoogleMapsCoords(string $map_url, string $map_host): array
{
